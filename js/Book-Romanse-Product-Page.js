...
{
    advContainer2 = JSON.parse(localStorage.getItem("rom"));
   console.log( romContainer2[2])
   typeComment.style.display = "none";
   sendComment.style.display = "none" ;
   makeSureIfLSEmpty();
  ...
  { 
         ...
         for (var i = 0 ; i < commentsContainer2.length ; i++)
         {
          ...
          <img src="../imges/bbbb.jpg" class="w-100 my-2" >
           ...
           <div  class="col-md-9">             
           <span class="my-2 displayEmpty">`+ romContainer2[i].rom+`</span>
           </div>`;
...
function hover(image) {
    image.setAttribute('src', 'imges/book img/rompr1-1.PNG');
  }
  
  function unhover(image) {
    image.setAttribute('src', 'imges/book img/rompr1-2.PNG');
  }
